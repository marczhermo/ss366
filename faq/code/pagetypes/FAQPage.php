...
class FAQPage_Controller extends Page_Controller {
     ...
     * @var int
     */
    public static $results_per_page = 3;
     ...
     * Otherwise runs a search and renders samplethe search results page.
    ...
    public function view() {
        $faq = $this->searchByID($this->request->param('ID'));
    ...
    /**
     * Using Swiftype to display a single record instead of fetching from database
     * @param  int $id FAQ ID
     * @return FAQ
     */
    public function searchByID($id)
    {
        $search = $this->createSearch('', 'FAQ', 'Swiftype');
        $search->filter(['ID' => (int) $id]);
        $search->setPageLength(1);
        return $search->fetch()->first();
    }
    ...
    public function search() {
        ...
        try {
            ...
            $results = $this->sampleSearch($keywords, $start, $limit);
    ...
    public function sampleSearch($keywords, $start, $limit)
    {
        //$request = $this->getRequest();
        // Controller method using the module's interface
        $search = $this->createSearch(
            $keywords, 'FAQ', 'Swiftype' //Swiftype
        );

        $search->setPageLength($limit);
        $search->setStartAt($start);
        //$pageNumber = 1 + floor($start/$limit);
        //$search->setPageNumber($pageNumber);

        $result = $search->fetch();

        $response = $search->getResponse();
        $pages = new PaginatedList($result, $this->getRequest());
        $pages->setTotalItems((int) $response['_total']);
        $pages->setPageLength($limit);
        $pages->setPageStart($start);
